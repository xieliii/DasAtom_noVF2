...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the 48-circuit ForceShuttle vs DasAtom benchmark pairwise on the same machine."
    )
    parser.add_argument(
        "--root",
        type=Path,
        default=Path(__file__).resolve().parent,
        help="Project root containing DasAtom, DasAtom_Origin, and local48_benchmark_list.txt.",
    )
    parser.add_argument(
        "--bundle-name",
        type=str,
        default=".tmp_local48_portable",
...
def main() -> int:
    ...
    list_path = root / "local48_benchmark_list.txt"
    ...
    # Prepare isolated per-circuit input folders so current and baseline read identical inputs.
